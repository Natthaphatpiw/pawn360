/**
 * Backtest: reproduce the production /api/estimate pipeline against the
 * Astly CM pawn-contract CSV, condition forced to 100% for every row.
 *
 ...
 *   computeRepresentativeUsedPriceTHB (REAL module, tsc-compiled) -> marketPrice
 *   marketPrice * 0.6 -> pawnPrice ; * condition(=1.0) ; snap to 500
 *
 * SerpAPI OFF (matches .env) and USE_TH_WEIGHTS=false -> live config.
 */
import fs from 'fs';
import path from 'path';
import OpenAI from 'openai';
import { computeRepresentativeUsedPriceTHB } from './backtest-price-representative.mjs';
...
ตอบกลับเป็น JSON เท่านั้น:
{
  ...
  const parsed = parseJsonFromText(getResponseText(response));
  return parsed?.productName?.trim() || `${brand} ${model}`.trim();
}

async function fetchWebSearchPrices(productName) {
...
- Use canonical product URLs and remove tracking parameters when possible.`;
  ...
  const runSearch = (maxTokens) => client.responses.create({
...
function computeFinal(marketPriceRep) {
...
async function main() {
  ...
  const keyOf = (r) => `${(r[COL.type] || '').trim()}|||${(r[COL.model] || '').trim()}`;
  ...
  await pool(toRun, 4, async (key) => {
    ...
    try {
      ...
      if (listings.length === 0) {
        ...
        console.log(`x ${model} -> no listings`);
      } else {
        ...
        cache[key] = { productName, listings, n: listings.length, marketPrice, pawnPrice, finalPrice, dispersionMode: analysis.mode };
        console.log(`ok ${model} (${productName}) n=${listings.length} market=${marketPrice} pawn@100%=${finalPrice} [${analysis.mode}]`);
      }
    } catch (e) {
      cache[key] = { productName: model, listings: [], n: 0, marketPrice: 0, pawnPrice: 0, finalPrice: 0, error: e?.message || String(e) };
      console.log(`x ${model} -> ERROR ${e?.message || e}`);
    }
  ...
  const newHeaders = ['ชื่อสินค้า (ระบบ normalize)', 'จำนวนรายการตลาดที่เจอ', 'ราคากลางระบบ (บาท)', 'ราคาจำนำระบบ @สภาพ100% (บาท)', 'ส่วนต่าง (จริง − ระบบ) (บาท)', 'ส่วนต่าง %', 'หมายเหตุ regime'];
  const outRows = [[...baseHeaders, ...newHeaders]];
  const summary = [];
  ...
  for (const r of dataRows) {
    ...
    const base = [r[COL.seq] || '', r[1] || '', r[2] || '', r[3] || '', r[4] || '', r[5] || '', r[6] || '', r[COL.type] || '', r[COL.model] || '', r[COL.price] || '', r[COL.redeemed] || ''];
    if (!dev || dev.error || dev.n === 0) {
      outRows.push([...base, dev?.productName || '', '0', '', '', '', '', dev?.error || 'no data']);
      summary.push({ seq: r[COL.seq], model: r[COL.model], actual, system: null, diff: null, pct: null });
      continue;
    }
    ...
    outRows.push([...base, dev.productName, String(dev.n), String(dev.marketPrice), String(system), diff === null ? '' : String(diff), pct === null ? '' : pct.toFixed(1), dev.dispersionMode || '']);
    summary.push({ seq: r[COL.seq], model: r[COL.model], actual, system, diff, pct });
  }
  ...
  const diffs = valid.map((s) => s.diff);
  ...
  const totalActual = sum(valid.map((s) => s.actual));
  const totalSystem = sum(valid.map((s) => s.system));
  const higher = valid.filter((s) => s.diff > 0).length;
  const lower = valid.filter((s) => s.diff < 0).length;
  const equal = valid.filter((s) => s.diff === 0).length;
  ...
  for (const s of summary) {
    console.log(`${String(s.seq).padStart(3)} | ${(s.model || '').padEnd(34).slice(0, 34)} | ${(s.actual ?? 0).toLocaleString().padStart(8)} | ${(s.system ?? 0).toLocaleString().padStart(8)} | ${(s.diff ?? 0).toLocaleString().padStart(8)} | ${s.pct === null ? '' : s.pct.toFixed(1)}`);
  }
}
