...
| Bank-transfer slips (payment / redemption / delivery-fee proofs) | Vercel Blob (configured region) | AML evidence + operational dispute window; financial-record aspect ties to AMLA | Financial-evidence slips: align to **5 years** (AML); other operational slips: **[set: e.g. N days]** after the dispute window | Scheduled Blob **deletion**; **anonymize** DB references |
| Item photographs (valuation / inspection / custody / return evidence) | Vercel Blob (configured region) | Operational + dispute/custody window (may incidentally contain identifiers) | **[set: e.g. N days]** after loan closure / return confirmation | Scheduled Blob **deletion** |
...
| **eKYC webhook inbox / notification outbox** (normalized status fields, opaque `uppass_slug`, hashed event key) | Supabase `ekyc_webhook_events` (server-only, RLS enabled, `anon`/`authenticated` revoked) | Operational durability and audit of identity-verification outcomes; contains **no** raw answers, document images, biometric payloads, form URLs, or LINE IDs | **[set: e.g. 180 days]** after the event reaches a terminal state, unless the outcome forms part of the AML 5-year KYC record | Scheduled delete; the verification *outcome* on the actor record follows the KYC row above |
| **eKYC attempt ledger** (actor reference, attempt status, provider slug, correlation key) | Supabase `ekyc_attempts` (server-only, RLS enabled) | Admission control and single-active-session integrity; links an attempt to an actor | **[set: e.g. 180 days]** after the attempt closes, or the AML 5-year window where it evidences a completed verification | Delete or anonymize the actor reference |
| **AI job state and payloads** (request parameters, canonical product text, references to Blob images) | Upstash (Redis), with payloads above 512 KiB in private Vercel Blob | Operational - the job must survive worker restarts and at-least-once redelivery | TTL-bounded; terminal jobs expire automatically | **Automatic TTL expiry**; Blob payloads deleted with the job |
| **Queue messages** | Vercel Queues | Transport only - each message carries **an opaque job or event id and nothing else** | Retention configured to **24 hours** (platform maximum 7 days) | Automatic expiry; no personal data present |
| **Application dead-letter index** (sanitized failure metadata, job id, error code) | Upstash (Redis) | Incident diagnosis for jobs that exhausted retry | **7 days** | Automatic TTL expiry |
| **AI usage / cost telemetry** (provider, model, token counts, latency, cache status, derived cost, hashed owner identifier) | Upstash (Redis) | Cost control and abuse detection; contains **no** prompts, images, or response bodies | `AI_USAGE_TTL_SECONDS`; daily aggregates retained for budgeting | Automatic TTL expiry |
| **Market price observations** (listing title, URL, price, source, evidence status) | Supabase `price_observations` | Business data about *products*, not people - it is the reuse cache that avoids repeat web searches | Read window bounded per query (days), rows pruned on schedule | Delete on schedule |
...
- **Propagation to Blob objects.** Any deletion or correction that affects stored media must reach the underlying Blob objects, not only the database references, so that no orphaned personal data remains in object storage. **To be implemented.**
...
3. **Execution.** Eligible data is deleted or anonymized across every store holding it (Supabase, MongoDB, Blob media, Upstash) and the deletion is propagated to processors (notably UPPASS) per their DPAs.
...
- **Engineering.** Implements and maintains the deletion/anonymization mechanisms (scheduled Blob purges, DSR-driven deletion, processor-deletion propagation) and keeps the constants in this schedule (cache TTLs, token TTL) accurate.
