export function computeRepresentativeUsedPriceTHB(items, cfg = {}) {
    ...
    const minCount = Math.max(config.minCountAbs, Math.ceil(config.minCountFrac * n));
    if (dispersionScore > config.dispersionThreshold) {
        let low = config.hiDispWindowLow;
        let high = config.hiDispWindowHighStart;
        let window = selectByPercentileRange(winsorizedPrices, low, high, weights);
    ...
    const window = selectByPercentileRange(winsorizedPrices, low, high, weights);
...
function validateWeights(weights, n) {
    if (weights.length !== n) {
...
function weightedPercentile(xs, ws, p) {
    if (xs.length === 0)
        throw new Error('weightedPercentile() of empty array');
    if (xs.length !== ws.length)
        throw new Error('xs and ws must have same length');
    const pp = clamp(p, 0, 100) / 100;
...
function selectByPercentileRange(xs, lowPct, highPct, weights) {
    const lo = weights
