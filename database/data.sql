...
CREATE TABLE public.pawn_delivery_requests (
  delivery_request_id uuid NOT NULL DEFAULT uuid_generate_v4(),
  contract_id uuid NOT NULL,
  loan_request_id uuid,
  customer_id uuid NOT NULL,
  drop_point_id uuid,
  ...
  status character varying DEFAULT 'DRIVER_SEARCH' CHECK (status::text = ANY (ARRAY['DRIVER_SEARCH'::character varying, 'PAYMENT_VERIFIED'::character varying, 'AWAITING_PAYMENT'::character varying, 'PAYMENT_REJECTED'::character varying, 'SLIP_UPLOADED'::character varying, 'DRIVER_ASSIGNED'::character varying, 'ITEM_PICKED'::character varying, 'ARRIVED'::character varying]::text[])),
  ...
  created_at timestamp without time zone DEFAULT now(),
  updated_at timestamp without time zone DEFAULT now(),
  CONSTRAINT pawn_delivery_requests_pkey PRIMARY KEY (delivery_request_id),
...
CREATE INDEX idx_pawn_delivery_requests_contract_id ON public.pawn_delivery_requests (contract_id);
CREATE INDEX idx_pawn_delivery_requests_status ON public.pawn_delivery_requests (status);
CREATE INDEX idx_pawn_delivery_requests_pawner_line_id ON public.pawn_delivery_requests (pawner_line_id);
CREATE INDEX idx_pawn_delivery_requests_drop_point_line_id ON public.pawn_delivery_requests (drop_point_line_id);
...
CREATE TABLE public.redemption_requests (
  ...
  reduction_amount numeric,
  increase_amount numeric,
  delivery_method character varying CHECK (delivery_method::text = ANY (ARRAY['SELF_PICKUP'::character varying, 'SELF_ARRANGE'::character varying, 'PLATFORM_ARRANGE'::character varying, 'DROPPOINT_SELF_PICKUP'::character varying, 'DROPPOINT_SELF_RIDER'::character varying, 'CENTRAL_SCHEDULE_7D'::character varying, 'CENTRAL_SELF_PICKUP_TODAY'::character varying, 'DROPPOINT_NEXT_DAY_PICKUP'::character varying]::text[])),
