    ...
    <div className={`min-h-screen bg-white ${sarabun.className}`}>
      ...
      <div className="p-4 max-w-md mx-auto">
        ...
        <button
          onClick={handleScan}
          disabled={!liffReady}
          className="w-full py-4 px-6 rounded-lg font-medium text-white text-lg disabled:opacity-50 disabled:cursor-not-allowed"
          style={{ backgroundColor: '#0A4215' }}
        >
          {liffReady ? 'แสกน QR Code' : 'กำลังโหลด...'}
        </button>

        {/* Footer Note */}
        <div className="text-center mt-4 text-xs text-gray-500">
          แสกน QR Code เพื่อตรวจสอบข้อมูล
        </div>
      </div>
    </div>
  );
